...
namespace Pavel.GUI.Visualizations {
    class ParallelPlotControl : OpenGLControl {
        ...
        private void CreateLineDisplayList() {
            ...
            Gl.glInitNames();
            Gl.glPushName(0);
            foreach (Point p in ps) { //TOOO: Replace with faster for loop
                if (cs.Contains(p)) {
                    ...
                    for (int colIndex = 0; colIndex < dimension; colIndex++) {
                        ...
                        x[colIndex, 1] = sy[colIndex] * ((float)p[map[colIndex]] + ty[colIndex]);
                    }
                    selectedPointValues.Add(x);
                } else {
                    //Set the color of the line with a current alpha-value
                    Gl.glColor4fv(ProjectController.GetSelectionColor(p).RGBwithA(vis.LinesAlpha * 0.01f));
                    Gl.glLoadName(pointName++);
                    Gl.glBegin(Gl.GL_LINE_STRIP);
                    for (int colIndex = 0; colIndex < dimension; colIndex++) {
                        Gl.glVertex3f(
                            tx[colIndex],
                            sy[colIndex] * ((float)p[map[colIndex]] + ty[colIndex]),
                            0f);
                    }
                    Gl.glEnd();
                }
